...
rewardStructs = cell(length(subjects),1); % Cell array to store the reward structures for each subject
for i=1:length(subjects)
    ...
    rewardStruct = struct();
    for s=1:nsessions
        rewardProbL = [subject_data.(sessions{s}).rewardProbL]';
        rewardProbR = [subject_data.(sessions{s}).rewardProbR]';
        rewardStruct.(sessions{s}) = struct('rewardProbL', rewardProbL, 'rewardProbR', rewardProbR);
    end
...
for iteration=1:num_iterations
    ...
    fields = fieldnames(this_data); % Extract the field names from the struct
    latent_att_cell = cellfun(@(field) [this_data.(field).latent_att], fields, 'UniformOutput', false); % Extract the "latent_att" values from all fields into a cell array
    latent_prob_cell = cellfun(@(field) [this_data.(field).latent_prob], fields, 'UniformOutput', false); % Extract the "latent_prob" values from all fields into a cell array
    latent_sim = [cat(2, latent_att_cell{:})' cat(2, latent_prob_cell{:})']; % Concatenate the "latent_att" values into a matrix
    ...
    parfor subj_it=1:num_subject_iterations
        this_data = feval(model.name, theta, rewardStructs{iteration}); % Generate simulated data based on the dynamic model and reward structure
        latent_sim_cell = cellfun(@(field) [this_data.(field).latent_att], fields, 'UniformOutput', false); % Extract the "latent_att" values from all fields into a cell array
