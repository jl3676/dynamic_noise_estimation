...
[out, idx] = sort(AICs(:, dynamic_model_ind) - AICs(:, static_model_ind));

% Create a figure for the plot
figure('Position', [300, 300, 1500, 800])

% Plot the parameter shift for each model parameter
for param = Ms{static_model_ind}.pnames(1:end-1)
    param_ind_1 = strcmp(Ms{static_model_ind}.pnames, param);
    param_ind_2 = strcmp(Ms{dynamic_model_ind}.pnames, param);
    subplot(4, 2, find(strcmp(param, Ms{static_model_ind}.pnames(1:end-1))))
    hold on
    [B1, I] = sort(All_Params{static_model_ind}(idx, param_ind_1));
    B2 = All_Params{dynamic_model_ind}(idx, param_ind_2);
    B2 = B2(I);
    for k = 1:length(B1)
    ...
    for k = 1:length(B1)
        ...
        if AICs(k, static_model_ind) > AICs(k, dynamic_model_ind)
    ...
    ylim([Ms{static_model_ind}.pMin(param_ind_1) Ms{static_model_ind}.pMax(param_ind_1)])
...
eps_ind = find(strcmp('epsilon', Ms{static_model_ind}.pnames));
lapse_ind = find(strcmp('lapse', Ms{dynamic_model_ind}.pnames));
...
h.Color = 'k';
hold on
plot(log(All_Params{static_model_ind}(:, eps_ind) + .001), log(All_Params{dynamic_model_ind}(:, lapse_ind) + .001), '.', 'MarkerSize', 20, 'MarkerEdgeColor', "#EDB120")
hold on
inds = AICs(:, dynamic_model_ind) - AICs(:, static_model_ind) > 0;
[r, p] = corr(All_Params{static_model_ind}(inds, end), All_Params{dynamic_model_ind}(inds, lapse_ind), 'type', 'Kendall');
inds = AICs(:, dynamic_model_ind) - AICs(:, static_model_ind) < 0;
plot(log(All_Params{static_model_ind}(inds, eps_ind) + .001), log(All_Params{dynamic_model_ind}(inds, lapse_ind) + .001), '.', 'MarkerSize', 20, 'MarkerEdgeColor', "#77AC30")
xlabel('log(epsilon)')
ylabel('log(lapse)')
...
eps_ind = find(strcmp('epsilon', Ms{static_model_ind}.pnames));
rec_ind = find(strcmp('recover', Ms{dynamic_model_ind}.pnames));
...
h.Color = 'k';
hold on
plot(1 - All_Params{static_model_ind}(:, eps_ind), All_Params{dynamic_model_ind}(:, rec_ind), '.', 'MarkerSize', 20, 'MarkerEdgeColor', "#EDB120")
hold on
[r, p] = corr(1 - All_Params{static_model_ind}(:, eps_ind), All_Params{dynamic_model_ind}(:, rec_ind), 'type', 'Kendall');
inds = AICs(:, dynamic_model_ind) - AICs(:, static_model_ind) < 0;
plot(1 - All_Params{static_model_ind}(inds, eps_ind), All_Params{dynamic_model_ind}(inds, rec_ind), '.', 'MarkerSize', 20, 'MarkerEdgeColor', "#77AC30")
xlabel('1 - epsilon')
ylabel('rec')
legend({'', 'static better', 'dynamic better'}, 'Location', 'best')
axis square
...
hold on
inds = (AICs(:, dynamic_model_ind) - AICs(:, static_model_ind) >= 0);
to_plot_1 = ((All_Params{dynamic_model_ind}(inds, lapse_ind)) - (All_Params{static_model_ind}(inds, eps_ind)));
hold on
inds = (AICs(:, dynamic_model_ind) - AICs(:, static_model_ind) < 0);
to_plot_2 = ((All_Params{dynamic_model_ind}(inds, lapse_ind)) - (All_Params{static_model_ind}(inds, eps_ind)));
...
hold on
inds = (AICs(:, dynamic_model_ind) - AICs(:, static_model_ind) >= 0);
to_plot_1 = ((All_Params{dynamic_model_ind}(inds, rec_ind)) - 1 + (All_Params{static_model_ind}(inds, eps_ind)));
hold on
inds = (AICs(:, dynamic_model_ind) - AICs(:, static_model_ind) < 0);
to_plot_2 = ((All_Params{dynamic_model_ind}(inds, rec_ind)) - 1 + (All_Params{static_model_ind}(inds, eps_ind)));
p = signrank(to_plot_2, 0, 'tail', 'left');
disp(['p = ' num2str(p) ' (dynamic better)'])
hold on
g = [zeros(length(to_plot_1), 1); ones(length(to_plot_2), 1)];
