...
num_iterations = size(subjects,2);

f = waitbar(0, 'Starting'); % Start progress bar for iterations
total_iterations = size(subjects,2);
current_iteration = 0;
...
for iteration=1:num_iterations
    ...
    theta = fitted_params(iteration,1:end-1); % Get the fitted parameters for the current iteration
    K = fitted_params(iteration,end);
    this_data = feval(model.name, theta, K, realData); % Generate simulated data based on the dynamic model and reward structure
    this_data(isnan(this_data(:,1)),:) = [];

    latent_sim = this_data(:,6:7); 
    ...
    parfor subj_it=1:num_subject_iterations
        ...
        all_latent_sim{subj_it} = this_data(:,6); 
    end
    ...
    parfor this_it=1:num_subject_iterations
        ...
        this_latent_rec = feval([model.name '_latent'], theta, K, data); % Estimate the latent state using the dynamic model
