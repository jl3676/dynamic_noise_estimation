% RLWM Model Fitting Script
%
% This script performs model fitting for the RLWM data using two models: static_model and dynamic_model.
% The script loads the RLWM data for a specified number of trials, defines the priors for the model parameters,
...
% Load data
load ../data/TortoiseAndHareData.mat

% Get list of subjects
subjects = unique(Alldata.ID)';
...
stick_sample = @(x) unifrnd(-1, 1); % Sampling function for stick parameter
rho_sample = @(x) unifrnd(0, 1); % Sampling function for rho parameter
...
curr_model.pdfs = {alpha_sample, bias_sample, stick_sample, rho_sample, forget_sample, eps_sample};
curr_model.pnames = {'alpha','bias','stick','rho','forget','epsilon','K'};
Ms{1} = curr_model;
...
curr_model.pdfs = {alpha_sample, bias_sample, stick_sample, rho_sample, forget_sample, lapse_sample, rec_sample};
curr_model.pnames = {'alpha','bias','stick','rho','forget','lapse','recover','K'};
Ms{2} = curr_model;
...
for m = 1:length(Ms)
    ...
    parfor k = 1:length(subjects) % parallel processing
        ...
        for K = 2:5
            myfitfun = @(p) feval([fit_model.name, '_llh'], p, K, data);
